...
class StartCommand(CommandHandler):
    ...
    def handle(self, chat_id, username):
        try:
            ...
            UserBotConversation.objects.update_or_create(
                user=user,
                chat=chat_id
            )
...
class InformCommand(CommandHandler):
    ...
    OPTIONS = [
        ...
        ('Alguém estranho no seu carro', 4,
            'Ei, tem alguém estranho rondando o seu carro.'),
        ('Mal estacionado', 5, 'Seu carro está mal estacionado e\
                                pode estar atrapalhando alguém a sair.'),
    ]
